...
MODEL_PATH    = "model-5/bigs.pkl"        # your big‐only model
...
# ─── Keep only bigs ──────────────────────────────────────────────────────────
df_test = df_test[df_test["POS"].apply(is_big)].copy()
if df_test.empty:
    raise ValueError(f"No bigs (C or pure PF) in Draft Years {TEST_YEARS}.")
...
print(f"\n=== Predicted NBA Career Scores (Bigs Only) ===\n")
